...
static const char help_msg[] =
    R"([?] Usage:
    $ sudo numactl -C [core] -m [memory] ./validate_mapping [OPTIONS]
    ...
    Options:
      ...
      --row,       -R [HEX]     DRAM row bits
      --column,    -C [HEX]     DRAM column bits
...
int main(int argc, char* argv[]) {
  ...
  uint64_t num_pages = 19, page_size = 1ULL * 1024ULL * 1024ULL * 1024ULL,
           ...
           dq_width = 8;
  uint64_t row_bits = 0, column_bits = 0;
  ...
  if (getuid() != 0) {
    spdlog::error("validate_mapping requires sudo privilege.");
    exit(EXIT_FAILURE);
  }
  ...
  if (argc < 2) {
    ...
    int opt, idx;
    while ((opt = getopt_long(argc, argv, "o:p:t:n:s:r:w:f:R:C:dvlh",
                              long_options, &idx)) != -1) {
      switch (opt) {
        case 'o':
          if (optarg) {
  ...
  MemoryPoolConfig* memory_pool_config =
      new MemoryPoolConfig(page_size, num_pages, granularity, true /* huge */);
  ...
  sudoku->SetRowBits(row_bits);
  sudoku->SetColumnBits(column_bits);
  spdlog::info("[+] Validate input DRAM address mapping");
  bool pass = sudoku->ValidateAddressMapping();
  if (!pass) {
    spdlog::error("[-] Input DRAM address mapping is not valid.");
  }
  sudoku->Finalize();
