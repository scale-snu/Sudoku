...
static const char help_msg[] =
    R"([?] Usage: 
    $ sudo numactl -C [core] -m [memory] ./watch_refreshes [OPTIONS]

    Options:
      --output,    -o [STR]     Output filename prefix
      --pages,     -p [INT]     Number of OS memory pages to allocate
      --mode,      -m [STR]     Mode (supported: watch, stat, and check)
        watch:                  Watch periodic refresh-induced latency spikes on single and paired memory accesses
...
int main(int argc, char* argv[]) {
  std::string fname_prefix = "default", type = "ddr4", mode = "watch";
  ...
  if (getuid() != 0) {
    spdlog::error("watch_refreshes requires sudo privilege.");
    exit(EXIT_FAILURE);
  }
  ...
  if (mode == "watch" || mode == "WATCH") {
    // Watch -- single
    spdlog::info("[+] WatchRefreshSingleAccess");
    sudoku->WatchRefreshSingleAccess();
    // Watch -- paired
    spdlog::info("[+] WatchRefreshPairedAccessCoarse");
    sudoku->WatchRefreshPairedAccessCoarse();
    spdlog::info("[+] WatchRefreshPairedAccessFine");
    sudoku->WatchRefreshPairedAccessFine();
  } else if (mode == "stat" || mode == "STAT") {
    // Stat -- single
    spdlog::info("[+] StatRefIntervalSingleAccess");
    sudoku->StatRefIntervalSingleAccess(REFRESH_CYCLE_LOWER_BOUND);
    // Stat -- paired
    spdlog::info("[+] StatRefIntervalPairedAccessCoarse");
    sudoku->StatRefIntervalPairedAccessCoarse(REFRESH_CYCLE_LOWER_BOUND);
    spdlog::info("[+] StatRefIntervalPairedAccessFine");
    sudoku->StatRefIntervalPairedAccessFine(REFRESH_CYCLE_LOWER_BOUND);
  } else if (mode == "check" || mode == "CHECK") {
    // Check -- paired
    Constraints c =
        Constraints(same_functions, diff_functions, row_bits, column_bits);
    spdlog::info("[+] CheckRefIntervalPairedAccessCoarse");
    sudoku->CheckRefIntervalPairedAccessCoarse(c, REFRESH_CYCLE_LOWER_BOUND);
    spdlog::info("[+] CheckRefIntervalPairedAccessFine");
    sudoku->CheckRefIntervalPairedAccessFine(c, REFRESH_CYCLE_LOWER_BOUND);
  } else {
